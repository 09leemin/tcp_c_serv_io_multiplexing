...
int fcntl_setnb(int fd);        /* set non-blokiong mode */
int add_ev(int efd, int fd);    /* add epoll variable to epollfd */
int del_ev(int efd, int fd);    /* delete epoll variable from epollfd */
...
int main(int argc, char **argv)
{
    ...
    listen(fd_listener, LISTEN_BACKLIOG);
...
int del_ev(int efd, int fd)
{
    if (epoll_ctl(efd, EPOLL_CTL_DEL, fd, NULL) == -1) {
        pr_out("fd(%d) EPOLL_CTL_DEL_Error(%d:%s)", fd, errno, strerror(errno));
        return -1;
    }
    close(fd);
    return 0;
